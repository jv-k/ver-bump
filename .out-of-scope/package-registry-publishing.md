...
VerBump does not publish packages to registries (npm, pnpm, yarn, or any
...
well served without VerBump in the loop:

- **Locally**, `npm publish` is a single command. `VerBump -p && npm publish`
...
- **In CI**, the modern pattern is tokenless OIDC trusted publishing triggered
  ...
  the `publish-npm` job in `.github/workflows/ci.yml`). VerBump's `--release`
...
All of that runs against VerBump's identity: a bash tool with git and jq as
