...
The domain language of `ver-bump`, an opinionated release tool for Git
...
What `ver-bump` is: a plain-bash release tool for Git repositories with no Node
...
**Default path**:
The execution of a `ver-bump` invocation that uses no opt-in feature flags. It
