...
| --- | --- | --- |
| R-COMP-1 | Emitted bash passes `bash -n`; zsh passes `zsh -n`; fish passes `fish -n` when available. | ✅ shipped — `test/completions-syntax.bats` |
...
| R-COMP-5 | Registered for both `ver-bump` and `ver-bump.sh` names. | ✅ shipped |
