...
| R-YES-3 | `FLAG_YES` is **not** a supported `.ver-bumprc` key — auto-confirmation is an explicit per-invocation choice, never a repo default. | ✅ by design (`_CONFIG_KEYS` excludes it) |
