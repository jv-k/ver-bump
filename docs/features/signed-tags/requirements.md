...
([#68](https://github.com/jv-k/VerBump/issues/68)): `--sign` (or
...
| --- | --- | --- | --- |
| R-SIGN-1 | `--sign` (boolean, long-only) and the `TAG_SIGN=true` config/env key (precedence per R-CFG-3: CLI > env > `.verbumprc` > default `false`): `do-tag` uses `git tag -s` instead of `-a`. Default path is unchanged (`-a`). | ✅ | `signed-tags.bats`, `args.bats` |
