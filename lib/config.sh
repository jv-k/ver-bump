...
#
# Safety: shell-sourced files are code. A world-writable rc is refused
# with exit code 3.
...
# Refuse to load a world-writable rc. Exits 3 via fail() if unsafe.
_assert-rc-safe() {
  local rc=$1
  if find "$rc" -perm -0002 -type f 2>/dev/null | grep -q .; then
    fail 3 \
      ".ver-bumprc at $rc is world-writable — refusing to load." \
