...
# release was cut from. Resolution order: --base / PR_BASE (env/.verbumprc) >
...
#   Default: notes generated by `gh release create --generate-notes` — no
#   dependency beyond the already-required gh — anchored with
#   --notes-start-tag to this series' previous tag when it exists. Under a
#   package scope the default is instead the package's own changelog entry
#   via render-release-notes (R-MONO-9). Setting $VERBUMP_RELEASE_NOTES_CMD
#   beats both: it runs that command and passes its stdout via `--notes`.
# Honours FLAG_DRYRUN by printing the resolved `gh release create` invocation
# to stderr instead of executing it. A custom notes command runs in dry-run too
# so the preview reflects real output (R-REL-5); the default path runs nothing.
# If a custom notes command exits non-zero, abort before calling `gh` (exit 1)
# and surface its stderr — the tag push already happened (live path) and
# is intentionally NOT rolled back (R-REL-6).
do-github-release() {
  [ "${DO_RELEASE:-false}" = true ] || return 0
  ...
  # Empty default → gh's own --generate-notes (no node/npx). A custom command
  # switches to capturing its stdout and passing it via --notes.
  notes_cmd="${VERBUMP_RELEASE_NOTES_CMD:-}"
  ...
  # Notes source, in precedence order (R-MONO-9): a custom command beats
  # everything; under a package scope the default is the package's own
  # changelog entry (gh's generator can't path-scope — it would list foreign
  # packages' PRs); otherwise gh --generate-notes, anchored to this tag
  # series' previous tag so GitHub can't auto-pick a baseline from another
  # series (amends ADR-18).
  local -a notes_args
  local notes_kind start_tag=""
  if [ -n "$notes_cmd" ]; then
    # Custom notes command: capture stdout (notes) and stderr separately so we
    # can surface a useful error when it fails. `eval` so users can pass
    # pipelines / arg lists via the env var (e.g. 'echo X | tr a-z A-Z').
    notes_err=$(mktemp)
    ...
    if [ "$notes_rc" -ne 0 ]; then
      ...
      fail 1 \
        ...
        "Fix the notes command, or unset VERBUMP_RELEASE_NOTES_CMD to use gh --generate-notes."
    fi
    rm -f "$notes_err"
    notes_args=(--notes "$notes")
    notes_kind="command"
  elif [ "${VB_SCOPE_ACTIVE:-false}" = true ]; then
    notes=$(render-release-notes)
    notes_args=(--notes "$notes")
    notes_kind="changelog"
  else
    notes_args=(--generate-notes)
    notes_kind="generated"
    if [ -n "${V_PREV:-}" ] \
       && git rev-parse --verify --quiet "refs/tags/${TAG_PREFIX}${V_PREV}" >/dev/null; then
      start_tag="${TAG_PREFIX}${V_PREV}"
      notes_args+=(--notes-start-tag "$start_tag")
    fi
  fi

  if [ "${FLAG_DRYRUN:-false}" = true ]; then
    # notes: "generated" (gh --generate-notes) vs "changelog" (the scoped
    # package entry) vs "command" (a custom VERBUMP_RELEASE_NOTES_CMD ran).
    # prerelease is a real boolean.
    if [ "${FLAG_JSON:-false}" = true ]; then
      record-effect-raw "$(jq -nc \
        --arg tag "$tag" \
        --arg notes "$notes_kind" \
        --argjson prerelease "$([ -n "$pre_flag" ] && echo true || echo false)" \
        '{action:"github-release", tag:$tag, notes:$notes, prerelease:$prerelease}')"
    fi
    local notes_preview="--generate-notes${start_tag:+ --notes-start-tag ${start_tag}}"
    [ "$notes_kind" != "generated" ] && notes_preview="--notes '${notes}'"
  ...
  local gh_msg gh_rc
  local -a gh_args=("$tag" "${notes_args[@]}")
...
do-pr() {
  ...
  # Package scope (R-MONO-3): same restriction as the changelog scan.
  local -a scope_args=()
  [ "${VB_SCOPE_ACTIVE:-false}" = true ] && scope_args=(-- "${VB_SCOPE_PATHS[@]}")
  RANGE=$([ "$(git tag -l "${TAG_PREFIX}${V_PREV}")" ] && echo "${TAG_PREFIX}${V_PREV}..HEAD")
  # shellcheck disable=SC2086
  body=$( git log --pretty=format:"- %s" ${RANGE} ${scope_args[@]+"${scope_args[@]}"} 2>/dev/null )
