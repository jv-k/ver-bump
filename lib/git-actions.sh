...
#   dependency beyond the already-required gh. Setting $VER_BUMP_RELEASE_NOTES_CMD
...
do-github-release() {
  ...
  notes_cmd="${VER_BUMP_RELEASE_NOTES_CMD:-}"
  ...
  if [ -n "$notes_cmd" ]; then
    ...
    if [ "$notes_rc" -ne 0 ]; then
      ...
      fail 1 \
        ...
        "Fix the notes command, or unset VER_BUMP_RELEASE_NOTES_CMD to use gh --generate-notes."
    fi
    rm -f "$notes_err"
