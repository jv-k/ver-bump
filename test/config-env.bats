...
@test "env TAG_PREFIX wins over .verbumprc (end-to-end)" {
  local repo
  repo="$(scratch_repo)"
  cd "$repo"
...
@test ".verbumprc wins over the builtin default when env is unset (end-to-end)" {
  local repo
  repo="$(scratch_repo)"
  cd "$repo"
  printf 'TAG_PREFIX=from-file\n' > "$repo/.verbumprc"
...
@test "builtin default is used when neither env nor .verbumprc set a value (end-to-end)" {
  local repo
  repo="$(scratch_repo)"
  cd "$repo"
  printf '{ "version": "1.0.0" }\n' > "$repo/package.json"
